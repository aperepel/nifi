...
public class JMSConnectionFactoryProvider extends AbstractControllerService implements JMSConnectionFactoryProviderDefinition {
    ...
    static {
        propertyDescriptors = Collections.unmodifiableList(Arrays.asList(CONNECTION_FACTORY_IMPL, CLIENT_LIB_DIR_PATH, BROKER_URI));
    }
                ...
                if (propertyName.equals(BROKER)) {
                    if (context.getProperty(CONNECTION_FACTORY_IMPL).getValue().startsWith("org.apache.activemq")) {
                        ...
                        if (hostPort.length != 2) {
                            throw new IllegalArgumentException("Unless you are using ActiveMQ, the 'Broker URI' "
                                    + "property must be of form 'host:port'. Was '" + entry.getValue()
                                    + "'. We understand that this may not be adequate with your JMS vendor. "
                                    + "Please file feature request at https://issues.apache.org/jira/browse/NIFI so "
                                    + "such support could be added.");
                        }
        ...
        if (method != null) {
            try {
                ...
                if (String.class.isAssignableFrom(returnType)) {
                    ...
                    method.invoke(this.connectionFactory, Integer.parseInt(propertyValue));
                } else if (long.class.isAssignableFrom(returnType)) {
                    method.invoke(this.connectionFactory, Long.parseLong(propertyValue));
                } else if (boolean.class.isAssignableFrom(returnType)) {
                    method.invoke(this.connectionFactory, Boolean.parseBoolean(propertyValue));
                } else {
                    throw new IllegalArgumentException("Unrecognized property type for " + propertyName);
                }
            } catch (Exception e) {
